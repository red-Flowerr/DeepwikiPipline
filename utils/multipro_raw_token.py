...
ENCODING = "cl100k_base"  # 可改 "o200k_base"
...
class Task:
    kind: str  # "deepwiki_txt" | "narratives_json"
    folder: str
    filepath: str
...
def worker(task_q, progress_q, encoding: str):
    import tiktoken
...
def main():
    ...
    if not tasks:
        raise SystemExit(
            f"No tasks found under {args.base}. Expected '*_deepwiki.txt' and/or '*_narratives.json' in each subfolder."
        )
    ...
    procs = []
    for _ in range(args.workers):
        p = ctx.Process(target=worker, args=(task_q, progress_q, args.encoding))
        p.start()
        procs.append(p)
    ...
    pbar = tqdm(
        total=len(tasks),
        desc=f"Tokenizing (tiktoken:{args.encoding})",
        unit="file",
        dynamic_ncols=True,
